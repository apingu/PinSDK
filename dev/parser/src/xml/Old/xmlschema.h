...
public:
	...
	XSElement(const basestring &name, const basestring &field,
	 ...
	 */
    static SimpleType StringToSimpleType(const basestring &type_name);
	 ...
	 * is taken from m_FieldName, and the value part is from m_Text of XMLNode.
	 ...
	 * @note The string m_Text of XMLNode will be converted by the element's type
	 * to match the value format of database.
	 */
	bool NodeToString(const XMLNode &node, basestring &result, bool bNoField=false);
...
private:
	...
	basestring m_Name;
	basestring m_FieldName;
	basestring m_TableName;
	const TCHAR* m_pColumnName;
	basestring m_JoinFieldName;
	basestring m_TypeName;
	SimpleType m_SimpleType;
	XSComplexType *m_pComplexType;
	basestring m_ParentTypeName;
	basestring m_ParentKeyName;
	XSElement *m_pParent;
	bool m_bIndex;
...
public:
	...
	/** Constructor by name. */
	XSComplexType(const basestring &name);
	...
	void BuildTableNameList(std::vector<basestring> &names, bool bCleanFirst);
	...
	bool DeleteElement(const basestring &name);

	/** Get number of elements in complex-type objects. */
	int GetElementCount() const { return m_Elements.size(); }

	/** Get the element object by name. */
	XSElement *GetElement(const basestring &name);

	/** Get the element which has relationship with given parent type. */
	XSElement *FindElementByParentType(const basestring &parent_type);
...
private:
	...
	std::vector<basestring> m_TableNames;
	XSDocument *m_pOwner;
...
public:
	 ...
	 */
	bool CreateSelectCommand(XMLNode &xml, basestring &cmd, bool bFirstSelect=false);
	 ...
	 */
	bool CreateUpdateCommand(XMLNode &xml, basestring &cmd);
	 ...
	 */
	bool CreateInsertCommand(XMLNode &xml, basestring &cmd);
	 ...
	 */
	bool CreateDeleteCommand(XMLNode &xml, basestring &cmd);
	...
	bool DeleteComplexType(const basestring &name);
	...
	XSComplexType *GetComplexType(const basestring &name);
