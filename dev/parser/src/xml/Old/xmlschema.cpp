...
XSElement::XSElement(const basestring &name, const basestring &field,
					 const basestring &type_name, bool is_index):
m_Name(name),
m_FieldName(field),
...
bool XSElement::Serialize(XMLNode &xml, bool is_storing)
{
	if(is_storing)
	{
		xml.m_Name = _T("element");
		xml[_T("name")].m_Text = m_Name;
		xml[_T("type")].m_Text = m_TypeName;

		if(m_pComplexType==NULL)
		{
			xml[_T("field")].m_Text = m_FieldName;
			
			if(IsJoined())  // save joined-field
			{
				XMLNode node;
				node.m_Name = _T("field");
				node.m_Text = m_JoinFieldName;
				xml.m_ChildNodes.push_back(node);
			}

			xml[_T("index")].m_Text = m_bIndex ? _T("true"):_T("false");
			
			// save relationship
			if(m_pParent)
			{
				XMLNode &node = xml[_T("parent")];
				node[_T("type")].m_Text = m_ParentTypeName;
				node[_T("element")].m_Text = m_pParent->m_Name;
			}
		}
		...
		if((node=xml.FindNode(_T("name")))!=NULL)
		{
			...
			if((node=xml.FindNode(_T("type")))!=NULL)
			{
				...
				if((node=xml.FindNode(_T("index")))!=NULL)
					m_bIndex = (node->m_Text==_T("true"));

				// load relationship
				if((node=xml.FindNode(_T("parent")))!=NULL)
				{
					m_ParentTypeName = node->operator[](_T("type")).m_Text;
					m_ParentKeyName = node->operator[](_T("element")).m_Text;
				}

				// load field name if it's simple type
				if((node=xml.FindNode(_T("field")))!=NULL)
				{
					m_FieldName = node->m_Text;

					// check if there's a join field
					node = xml.FindNextNode(_T("field"));
					if(node) m_JoinFieldName = node->m_Text;
...
XSElement::SimpleType XSElement::StringToSimpleType(const basestring &type_name)
{
...
bool XSElement::NodeToString(const XMLNode &node, basestring &result, bool bNoField)
{
	if(m_Name==node.m_Name && m_SimpleType!=stUnknown)
	{
		...
		switch(m_SimpleType)
		{
			...
			default: result += node.m_Text;  // no need to add quotes
		}
...
// Constructor by name
XSComplexType::XSComplexType(const basestring &name):
m_Name(name),
m_pOwner(NULL)
{}
...
bool XSComplexType::Serialize(XMLNode &xml, bool is_storing)
{
	...
	else
	{
		...
		if((node=xml.FindNode(_T("name")))!=NULL)
		{
			...
			for(unsigned int i=0; i< xml.m_ChildNodes.size(); i++)
			{
				XMLNode &node = xml.m_ChildNodes[i];
				if(node.m_Name==_T("element"))
				{
					XSElement xse;
					if(xse.Serialize(node, false) && GetElement(xse.m_Name)==NULL)
					{
...
int XSComplexType::SaveConditionsToStream(OutStream &outs, XMLNode &xml, int options, const TCHAR* filter, int nCons)
{
	...
	for(unsigned int i=0; i< m_Elements.size(); i++)
	{
		...
		if((node=xml.FindNode(xse.m_Name))!=NULL)
		{
			if(xse.IsSimpleType())
			{
...
void XSComplexType::ConvertFlatXML(XMLNode &flat, XMLNode &result)
{
	...
	for(unsigned int i=0; i< m_Elements.size(); i++)
	{
		...
		if(xse.IsSimpleType())
		{
			...
			{
				result.m_ChildNodes.push_back(XMLNode());
				XMLNode &new_node = result.m_ChildNodes.back();
				new_node.m_Name = xse.m_Name;
				new_node.m_Text = node->m_Text;
			}
		}
		else if(xse.IsComplexType())
		{
...
// Get element by name
XSElement *XSComplexType::GetElement(const basestring &name)
{
	XSElement *xse = NULL;
...
XSElement *XSComplexType::FindElementByParentType(const basestring &parent_type)
{
	XSElement *xse = NULL;
...
void XSComplexType::BuildTableNameList(std::vector<basestring> &names, bool bCleanFirst)
{
...
{
	XSElement *p = GetElement(xse.m_Name);
...
// Add an element into complex type
bool XSComplexType::DeleteElement(const basestring &name)
{
	for(unsigned int i=0; i< m_Elements.size(); i++)
	{
		if(name==m_Elements[i].m_Name)
		{
...
void XSDocument::Serialize(XMLNode &xml, bool is_storing)
{
	...
	if(is_storing)
	{
		xml.m_Name = _T("schema");
		xml.m_ChildNodes.clear();
	...
	else
	{
		...
		for(i=0; i< xml.m_ChildNodes.size(); i++)
		{
			XMLNode &node = xml.m_ChildNodes[i];
			if(node.m_Name==_T("complexType"))
			{
				XSComplexType xsc;
				if(xsc.Serialize(node, false) && GetComplexType(xsc.m_Name)==NULL)
				{
...
XSComplexType *XSDocument::GetComplexType(const basestring &name)
{
	XSComplexType *xsc = NULL;

	for(unsigned int i=0; i< m_ComplexTypes.size(); i++)
		if(name==m_ComplexTypes[i].m_Name)
		{
...
bool XSDocument::CreateSelectCommand(XMLNode &xml, basestring &cmd, bool bFirstSelect)
{
	bool retval = false;
	XSComplexType *xsc = GetComplexType(xml.m_Name);
	cmd = _T("");
...
bool XSDocument::CreateUpdateCommand(XMLNode &xml, basestring &cmd)
{
	int nUpdates = 0;
	XSComplexType *xsc = GetComplexType(xml.m_Name);
	cmd = _T("");
...
bool XSDocument::CreateInsertCommand(XMLNode &xml, basestring &cmd)
{
	int nInserts = 0;
	XSComplexType *xsc = GetComplexType(xml.m_Name);
	cmd = _T("");
...
bool XSDocument::CreateDeleteCommand(XMLNode &xml, basestring &cmd)
{
	int nDeletes = 0;
	XSComplexType *xsc = GetComplexType(xml.m_Name);
	cmd = _T("");
...
{
	XSComplexType *xsc = GetComplexType(flat.m_Name);

	if(xsc)
	{
		result.m_Name = xsc->m_Name;
...
{
	XSComplexType *p = GetComplexType(xsc.m_Name);
...
bool XSDocument::DeleteComplexType(const basestring &name)
{
	for(unsigned int i=0; i< m_ComplexTypes.size(); i++)
	{
		if(name==m_ComplexTypes[i].m_Name)
