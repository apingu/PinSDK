...
namespace GSO
{
    ...
    */
    ...
    // DESC:
    //=============================================================================
    uint_t DataStream::ForAllCell (OnCellID cellcb, void* pData)
    {
        std::map<cellid_t, StreamCell*>::iterator iter = m_RealmBuf.begin();
        unsigned int i = 0;
    ...
    // DESC:
    //=============================================================================
    void DataStream::ForAllNeighborhood (cellid_t Id, OnCellID func, void* data)
    {
        if (func == NULL)
        { return; }
    ...
    */
    ...
    void DataStream::ForceUpdateCenter (cellid_t Id)
    {
        ...
        for (uint_t l = 0; l < m_WaitLoadSeq.size(); l++)
        {
            ...
            mstimen_t btime = pftml::Time::GetTickMSTime();
            OnLoadCell (celln->Id,
                        celln->Descriptor, //region->Name,
                        &celln->pData,
                        &celln->pUserData);
            mstimen_t atime = pftml::Time::GetTickMSTime();
    ...
    void DataStream::UpdateCenter (cellid_t Id)
    {
        ...
        if (m_FocusedCellId == Id)
        {
            if (!m_WaitLoadSeq.empty() )
            {
                ...
                mstimen_t atime = pftml::Time::GetTickMSTime();
                celln->LoadTime = atime - btime;
                Pf_INFO ( "DataStream::UpdateCenter: Load time: %d Size %d\n", celln->LoadTime, m_WaitLoadSeq.size() );
                m_pLoadedCellVec.push_back (celln);
...
#ifdef _DEBUG
        mstimen_t computertime = pftml::Time::GetTickMSTime();
#endif
        CellRealm* cell = (CellRealm*) GetStreamCell (Id);
        ...
        std::stable_sort (m_WaitLoadSeq.begin(),
                          ...
                          SortByLoadTime);
#ifdef _DEBUG
        Pf_INFO ( "DataStream::UpdateCenter: Compute time: %d\n",  pftml::Time::GetTickMSTime() - computertime );
#endif
        /*
