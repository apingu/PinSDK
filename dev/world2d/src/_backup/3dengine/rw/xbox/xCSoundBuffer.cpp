																		 ...
																		 DWORD dwBlkAlign,BOOL bStereo)
{
	...
	dsbdesc.dwFlags   =	0; //DSBCAPS_STATIC | DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME
	...
	if(hr != DS_OK)
  {
    ...
    case DSERR_ALLOCATED:
			DebugOutputString(NULL, "DSERR_ALLOCATED","DSOUND ERR",MB_OK);
      break;
    case DSERR_BUFFERTOOSMALL:
      MessageBox(NULL, "DSERR_BUFFERTOOSMALL","DSOUND ERR",MB_OK);
      break;
    case DSERR_BADFORMAT:
      MessageBox(NULL, "DSERR_BADFORMAT","DSOUND ERR",MB_OK);
      break;
    case DSERR_DS8_REQUIRED:
      MessageBox(NULL, "DSERR_DS8_REQUIRED","DSOUND ERR",MB_OK);
      break;
    case DSERR_INVALIDPARAM:
      MessageBox(NULL, "DSERR_INVALIDPARAM","DSOUND ERR",MB_OK);
      break;
    case DSERR_UNSUPPORTED:
      MessageBox(NULL, "DSERR_UNSUPPORTED","DSOUND ERR",MB_OK);
      break;
    case DSERR_OUTOFMEMORY:
      MessageBox(NULL, "DSERR_OUTOFMEMORY","DSOUND ERR",MB_OK);
      break;
    }
#endif
#ifdef _XBOX
//		DebugOutputString("DSOUND ERROR: %i", hr);
#endif
    return FALSE;
  }
...
//////////////////////////////////////////////////////////////////////////////////
BOOL CSoundBuffer::LoadFromWav(CSound* pDS, char *fname, HANDLE hFile, int Num)
{
	DWORD dwFilePos;
	HRESULT hr;
	int i;
  ...
  if(fname)
  {
    ...
    {
			char szMsg[256];
			wsprintf(szMsg,"CreateFile failed in CSoundBuffer::LoadFromWav. %s not loaded.\n",fname);
...
//		hr = pds->CreateSoundBuffer(Buffers[0], &Buffers[i], NULL);
		hr = 0;
		if FAILED(hr){
...
BOOL CSoundBuffer::LoadFromPakWav(CSound* pDS, char *fname, int numBuf)
{
  int i=0;
  ...
  file = fileFunctions->rwfopen(path, "rb");
  if(!file)
  {
		char szMsg[256];
		wsprintf(szMsg,"CSoundBuffer::LoadFromPakWav. %s not loaded.\n", fname);
		OutputDebugString(szMsg);
...
BOOL CSoundBuffer::ReadDataFromHandle(HANDLE hFile, DWORD dwSize)
{
	...
	if(dwData1Size > 0) {
		...
		if(dwReadSize!=dwData1Size){
			char holder[256];
			wsprintf(holder, "Data1 : %d, dwdata: %d, hFile: %d", pData1, dwData1Size, hFile);
