...
public:
	...
	BOOL CreateSoundBuffer(DWORD dwBufSize, DWORD dwFreq, DWORD dwSampleBits,
                         DWORD dwBlkAlign, BOOL bStereo);
	void FreeBuffers(void);
	LPDIRECTSOUNDBUFFER GetFreeBuffer(void);

  BOOL LoadFromPakWav(CSound* pDS, TCHAR *fname, int numBuf);
  BOOL LoadFromPakHandle(void* file);

	BOOL LoadFromWav(CSound* pDS, TCHAR* fname, HANDLE hFile = NULL, int Num = 1);
