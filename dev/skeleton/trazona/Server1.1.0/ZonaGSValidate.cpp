...
//#define EntityX( tz )        ( (TZMessageCreatureMove*)(tz) )->fPropData.fPos.x
//#define EntityY( tz )        ( (TZMessageCreatureMove*)(tz) )->fPropData.fPos.y
//#define EntityZ( tz )        ( (TZMessageCreatureMove*)(tz) )->fPropData.fPos.z
#define IsMoveStateType( e ) ( ((TZMessage*)e)->fType == TZMessage::kCreatureMoveState )
*/
...
int getRegionIdForEntityRecord(byte* masterEntityState)
{
	...
	//return getRegionIdByPos( MoveStateMessage( masterEntityState ).x,
	//	                     MoveStateMessage( masterEntityState ).y,
	//			             MoveStateMessage( masterEntityState ).z );
	return 0;
}
...
int onEntityJoinedGame(ZonaServerEntity *entity)
{
	...
	//int regionID = getRegionIdByPos( MoveStateMessage( entity->getPublicStateProperty() ).x,
	//	                             MoveStateMessage( entity->getPublicStateProperty() ).y,
	//								 MoveStateMessage( entity->getPublicStateProperty() ).z );//getRegionIdForEntityRecord( MoveState( masterEntityState ) );
...
void onValidateGameStateMsg(ZonaServerEntity *entity, byte* stateData,
							short stateDataSize)
{
	//DebugOutPutf(" validateGameStateMsg ", "Terazona" );
    //	publishGameStateMsg(eid, 0, CNVNC_CVRG_DEFAULT, PUB_OP_DEFAULT|PUB_OP_CORRECTION, stateData, stateDataSize);

	//if( publishGameStateMsgTo( eid, PUB_QOM_2, stateData, stateDataSize) == false )
	//	DebugOutPutf(" publishGameStateMsgTo failure ", "Terazona" );
	//if( publishGameStateMsg(eid, 0, PUB_QOM_2|PUB_OP_CORRECTION, stateData, stateDataSize) == false )
	//    DebugOutPutf(" publishGameStateMsg failure ", "Terazona" );
	...
	//	e->rID = getRegionIdForEntityRecord( send->buf );
	
	//int rID = getRegionIdForEntityRecord(stateData);
	//if( publishGameStateMsgInSphere( e->rID, PUB_QOM_2, PUB_OP_CORRECTION, send->buf, send->size ) == false )
	...
	//printf("[ Entity %d ] message size = %d rId = %d\n", eid, e->MessageSize(), rID );
	//fflush(stdout);
	...
	/*	
	for( int i = 0; i < gEntityMgr.EntitySize(); i++ )
	{
	    if( publishGameStateMsgTo( gEntityMgr[i]->eID, PUB_QOM_2, send->buf, send->size ) == false )
		{
	    	printf(" publishGameStateMsg failure " );
		    fflush(stdout);
		}
...
bool onEnterEntity(ZonaServerEntity *entity)
{
	...
	gEntityMgr.AddEntity( entity->getId(), (byte*)entity->getPublicDisplayProperty(), entity->getPublicDisplayPropertyLength() );
