...
// Special case for potentially quirky non-GDI drivers
        ...
        GetCursorPos(&pfstl);
        pfstl.x -= IconInfo.xHotspot;
        pfstl.y -= IconInfo.yHotspot;
        DrawIcon(hdcBackBuffer, pfstl.x, pfstl.y, MouseCursor);
    ...
    else
    {
        ...
        RECT rcSrceen;
        GetClientRect((HWND)hwndAppWindow, &rcSrceen);
        ClientToScreen((HWND)hwndAppWindow, (POINT*)&rcSrceen.left);
        ClientToScreen((HWND)hwndAppWindow, (POINT*)&rcSrceen.right);
        ddFrontBuffer->Blt(&rcSrceen, ddBackBuffer, NULL, DDBLT_WAIT, NULL);
    }
}
