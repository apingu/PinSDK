...
#include "crypt-crc_common.h"

// Static CRC table
namespace crc32
{
    dword_t crc_table[256] =
    {
    ...
    //***********************************************
    void CalcCrc32( const unsigned char byte, unsigned int& dwCrc32 )
    {
        dwCrc32 = ( ( dwCrc32 ) >> 8 ) ^ crc_table[ ( byte ) ^ ( ( dwCrc32 ) & 0x000000FF ) ];
    }
    ...
    //***********************************************
    //DWORD CCrcStatic::Crc32( const char* buffer, unsigned long buflen, unsigned long& dwCrc32 )
    unsigned int Encode( const char* buffer, unsigned int buflen )
    {
        ...
        //try
        {
            for ( uint_t i = 0; i < buflen; i++ )
            {
                CalcCrc32( (unsigned char)( buffer[ i ] ), dwCrc32 );
            }
            ...
            //          LPVOID ptrCrcTable = &crc_table;
            //
            //          //BYTE buffer[MAX_BUFFER_SIZE];
            //          //size32_t dwBytesRead = buflen;
            //          //UINT bSuccess = ReadFile(hFile, buffer, sizeof(buffer), &dwBytesRead, NULL);
            ...
            //
            //                  mov edi, ptrCrcTable        // Load the CRC32 table
            //
            //                  lea esi, buffer             // Load buffer
            //                  mov ebx, buflen         // Load dwBytesRead
