...
int OBPathSearch::Init( int* map, int width, int height )
{
    MapData = map;

    if ( ( MapWidth == width ) && ( MapHeight == height ) && ( MapSearch != NULL ) )
    {
        return 1;
    }
...
int OBPathSearch::SearchInMap( int x1, int y1, int x2, int y2 )
{
...
// Initilizing ----------------------------------------------------------
    ...
    memset( MapSearch, 0x7FFF7FFF, sizeof( unsigned short ) * MapSize );
...
void GetAround( int pos, int* around )
{
    ...
    for ( i = 0; i < Dircount; i++ )
    {
        ...
        else if ( ( MapData[j] > 0 ) && ( MapData[j] < 10000 ) )
        {
            around[i] = j;
        }
        else
    ...
    if ( around[0] == 0 )
    {
        around[4] = 0; around[7] = 0;
    }
    if ( around[1] == 0 )
    {
        around[4] = 0; around[5] = 0;
    }
    if ( around[2] == 0 )
    {
        around[5] = 0; around[6] = 0;
    }
    if ( around[3] == 0 )
    {
        around[6] = 0; around[7] = 0;
    }
}
...
int  OBPathSearch::Find( int x1, int y1, int x2, int y2 )
{
    ...
    int  ret = SearchInMap( x1, y1, x2, y2 );
