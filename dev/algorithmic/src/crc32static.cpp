...
bool CCrc32Static::GetFileSizeQW(const HANDLE hFile, QWORD &qwSize)
{
	...
	try
	{
		...
		if(dwLo == INVALID_FILE_SIZE && GetLastError() != CRCNO_ERROR)
		{
			bSuccess = false;
			qwSize = 0;
		}
...
//***********************************************
DWORD CCrc32Static::StringCrc32( const char* szString, DWORD &dwCrc32)
{
	assert(szString);

	DWORD dwErrorCode = CRCNO_ERROR;
...
//***********************************************
DWORD CCrc32Static::FileCrc32Streams( const char* szFilename, DWORD &dwCrc32)
{
#if UNICODE || _UNICODE
	...
	try
	{
		...
		if(file.is_open())
		{
			char buffer[MAX_CRCBUFFER_SIZE];
			int nLoop, nCount;
...
//***********************************************
DWORD CCrc32Static::FileCrc32Win32( const char* szFilename, DWORD &dwCrc32)
{
	assert(szFilename);
	assert(lstrlen(szFilename));
	...
	try
	{
		...
		{
			BYTE buffer[MAX_CRCBUFFER_SIZE];
...
//***********************************************
DWORD CCrc32Static::FileCrc32Filemap( const char* szFilename, DWORD &dwCrc32)
{
	assert(szFilename);
	assert(lstrlen(szFilename));
		...
		else
		{
			...
			else if(qwFileSize != 0)	// We cannot CRC zero byte files
			{
				...
				else
				{
					...
					while(qwFileSize > 0)
					{
						if(qwFileSize < MAX_CRCVIEW_SIZE)
							dwViewSize = LODWORD(qwFileSize);
						else
							dwViewSize = MAX_CRCVIEW_SIZE;
...
//***********************************************
DWORD CCrc32Static::FileCrc32Assembly( const char* szFilename, DWORD &dwCrc32)
{
	assert(szFilename);
	assert(lstrlen(szFilename));
