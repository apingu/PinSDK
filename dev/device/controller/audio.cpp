...
// Channel variables
typedef struct {
	int						  sound;		// ID of sound playing in this instance
...
static struct {
	...
	sound*					  soundlist;				// Pointer to currently loaded sound list
	int                       numsounds;				// Number of sounds in current sound list
...
void audio_init (int window, int device)
{
	...
	for (int i = 0; i < NUMCHANNELS; i++) _audio.channels[i].sound = -1;
...
)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		for (int j = 0; j < 24; j++)
		{
			...
			if (endpoints & (1 << j)) _audio.channels[(j<<1) + i].sound = -1;
		}
	}
	#endif
	...
	if (_audio.fadetime != 0)
	{
		...
		if (_audio.fadetime > 0)
		{
			...
			audio_setmastervolume (_audio.fadetime / _audio.fadelength);
		}
		else
		{
...
{
	// Make sure any previous sound lists have been unloaded
	assert (_audio.soundlist == 0);
	assert (_audio.numsounds == 0);

	// Store sound list information
	_audio.soundlist = soundlist;
	_audio.numsounds = sizeofsoundlist / sizeof(sound);
	...
	for (int i = 0; i < _audio.numsounds; i++)
	{
		//-----------------------------------------------------
		...
		*w = 0;
		...
		if (!FAILED(result))
		{
			...
			if (soundlist[i].flags & AUDIO_FLAG_LOOPING) ((IDirectMusicSegment8*) soundlist[i].data)->SetRepeats(DMUS_SEG_REPEAT_INFINITE);
...
void audio_unloadsounds (void)
{
	...
	#if defined(_WINDOWS) || defined(_XBOX)
	// Reference sound list
	sound* soundlist = _audio.soundlist;
	int    numsounds = _audio.numsounds;
...
/******************************************************************************
 ...
 (c) 2002 David O'Connor (procode@pacbell.net). All rights reserved.
******************************************************************************/
int audio_playsound		// returns channel sound is playing on, or -1 if playback failed
(
	int   sound,		// entry in sound list to play
...
{
	// Validate parameters
	assert (_audio.soundlist);
	assert ((sound >= 0) && (sound < _audio.numsounds));
	if ((sound < 0) || (sound >= _audio.numsounds)) return -1;

	// Exit if there is no sound data for this sound
	if (_audio.soundlist == 0) return -1;
	if (_audio.soundlist[sound].data == NULL) return -1;

	// If single instance flag is set then check active voices and exit if this sound is already playing
	if (_audio.soundlist[sound].flags & AUDIO_FLAG_SINGLE)
	{
		for (int i = 0; i < NUMCHANNELS; i++) if (_audio.channels[i].sound == sound) {return i;}
	}

	// Find a free channel, exit if none is available
	int channel = -1;
	for (int i = 0; i < NUMCHANNELS; i++) if (_audio.channels[i].sound == -1) {channel = i; break;}
	if (channel == -1) return -1;

	// Record the sound id for the channel
	_audio.channels[channel].sound = sound;
	...
	// Apply base volume from sound list
	volume *= _audio.soundlist[_audio.channels[channel].sound].volume;
	...
	// Get the base pitch from the sound list
	float pitch = _audio.soundlist[_audio.channels[channel].sound].pitch;
	...
	HRESULT result = _audio.performance8->PlaySegmentEx ((IDirectMusicSegment8*) _audio.soundlist[sound].data, NULL, NULL, DMUS_SEGF_SECONDARY, 0, &_audio.channels[channel].state, NULL, NULL);
	assert (result == S_OK);
	...
	audio_setpan    (channel, pan);
	audio_setpitch  (channel, pitch);
	#endif
	...
	sceSdRemote (1, rSdSetAddr, SD_VA_SSA + channel, _audio.soundlist[sound].data);
...
)
{
	...
	if (_audio.soundlist[_audio.channels[channel].sound].flags & AUDIO_FLAG_DOPPLER)
	{
...
)
{
	...
	int frequency = _audio.soundlist[_audio.channels[channel].sound].frequency;
...
)
{
	...
	// Stop channel
	if (_audio.channels[channel].sound != -1)
	{
		//-------------------------
		...
		_audio.channels[channel].sound = -1;
	}
}
...
void audio_stopall (void)
{
	...
	//------------------------------
	#if defined(_WINDOWS) || defined(_XBOX)
	for (int i = 0; i < NUMCHANNELS; i++) audio_stopsound(i);
	#endif
