...
BOOL CButtonCombo::Create( CRect rect,
                           CSize sizeGrid,
                           CSize sizeBtn,
                           CWnd* pParent,
                           UINT nID,
                           CString sTextItems )
{
    ...
    if( !m_wndColor.Create( rect.left,
                            rect.bottom,
                            sizeGrid,
                            sizeBtn,
                            this,
...
void CButtonCombo::OnItemSelected( long lItem,int iRealItem )
{
    ...
    if( m_lItem >= 0 )
    {
        if( m_pParent->GetOwner()->IsKindOf( RUNTIME_CLASS( CDialog ) ) )
            m_pParent->GetOwner()->SendMessage( WM_USER + 21,
                                                ( WPARAM ) m_nID,
                                                ( LPARAM ) m_lItem );
        else
        ...
        //m_pParent->SendDlgItemMessage(m_nID, WM_USER+21, (WPARAM)m_nID, (LPARAM)m_lItem);
...
void CButtonCombo::DrawArrow( CDC* pDC,CRect* pRect )
{
    ...
    CPoint ptCenter ( pRect->left + ( pRect->Width() / 2 ),
                        ...
                        CString sTextItems )
{
    ...
    int iWidth      = ( m_sizeGrid.cx* m_sizeBtn.cx ) +
                      ( m_sizeGrid.cx - 1 ) +
                      4; 
    int iRow        = ( sizeText.cy > 0 ) ? 0 : 1;
    int iHeight     = ( m_sizeGrid.cy* m_sizeBtn.cy ) +
                      ( m_sizeGrid.cy - iRow ) +
                      4 +
                      sizeText.cy; 
                        ...
                        CString sTextItems )
{
    ...
    CSize sizeText  = UpdateTextItems( sTextItems );
    int iWidth      = ( m_sizeGrid.cx * ( m_sizeBtn.cx + 8 ) ) +
                      ( m_sizeGrid.cx - 1 ) +
                      4; 
    int iRow        = ( sizeText.cy > 0 ) ? 0 : 1;
    int iHeight     = ( m_sizeGrid.cy * ( m_sizeBtn.cy + 8 ) ) +
                      ( m_sizeGrid.cy - iRow ) +
                      4 +
                      sizeText.cy; 
    CRect rect      ( x, y, x + iWidth, y + iHeight );
...
void CWndCombo::OnLButtonUp( UINT nFlags,CPoint point )
{
    ...
    {
        if( m_bmImage.m_hObject )
            ( ( CButtonCombo * ) m_pParent )->OnItemSelected( m_iSelectedItem,
                                                              m_iSelectedItem );
        else
            ( ( CButtonCombo * ) m_pParent )->OnItemSelected( GetColor( m_iSelectedItem ),
                                                              m_iSelectedItem );
    }
    m_iSelectedItem = -1;
    ShowWindow( SW_HIDE );
...
void CWndCombo::DrawBtnFace( CDC* pDC,CRect rect,long lItem )
{
    ...
    if( m_bmImage.m_hObject )
    {
        CPoint ptCenter ( rect.left + ( rect.Width() / 2 ),
                          rect.top + ( rect.Height() / 2 ) );
        CPoint ptOffSet ( ptCenter.x - ( m_sizeBtn.cx / 2 ),
                          ptCenter.y - ( m_sizeBtn.cy / 2 ) );

        CDC memDC;
        memDC.CreateCompatibleDC( pDC );
        CBitmap* pOldBitmap = memDC.SelectObject( &m_bmImage );

        pDC->BitBlt( ptOffSet.x,
                     ptOffSet.y,
                     m_sizeBtn.cx,
                     m_sizeBtn.cy,
                     &memDC,
                     ( int ) lItem * m_sizeBtn.cx,
                     0,
                     SRCCOPY );
        memDC.SelectObject( pOldBitmap );
    }else
    {
        CBrush brRect   ( lItem );
        pDC->FillRect( CRect( rect.left + 2,
                              rect.top + 2,
                              rect.right - 3,
                              rect.bottom - 2 ),
                       &brRect );
...
void CWndCombo::OnPaint()
{
    ...
    for( int i = 0;i < m_sizeGrid.cy;i++ )
    {
        ...
        for( int j = 0;j < m_sizeGrid.cx;j++ )
        {
            ...
            if( m_bmImage.m_hObject )
            {
                ...
                if( iItem < m_iNbImages )
                {
                    CPoint ptCenter ( rcBtn.left + ( rcBtn.Width() / 2 ),
            ...
            }else
            {
                ...
                memDC.FillRect( CRect( rcBtn.left + 1,
                                       rcBtn.top + 1,
                                       rcBtn.right - 1,
                                       rcBtn.bottom - 1 ),
                                &brRect );
            ...
            if( iItem == m_iSelectedItem )
                memDC.FrameRect( CRect( rcBtn.left - 1,
                                        rcBtn.top - 1,
                                        rcBtn.right + 1,
                                        rcBtn.bottom + 1 ),
                                 &brBlack );
    ...
    if( m_lstTextItems.GetSize() > 0 )
    {
        ...
        if( m_iSelectedItem != -1 )
        {
            ...
            memDC.DrawText( m_lstTextItems[ m_iSelectedItem ],
                            -1,
                            rcText,
...
COLORREF CWndCombo::GetColor( int iIndex )
{
    ...
    UINT uResult        = GetSystemPaletteEntries( hDC,
                                                   0,
                                                   NUMENTRIES,
                                                   palette );        
    ::ReleaseDC( NULL, hDC );
    return RGB( palette[ iIndex ].peRed,
                palette[ iIndex ].peGreen,
                palette[ iIndex ].peBlue );
}   
