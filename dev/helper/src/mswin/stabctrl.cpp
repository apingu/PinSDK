...
CSTabCtrl::CSTabPage::CSTabPage(CSTabCtrl* pare)
{
	m_Parent = pare;
...
BOOL CSTabCtrl::CSTabPage::ShowWindows ( INT _nCmdShow )
{
	...
	::GetWindowRect( m_Parent->GetSafeHwnd(), &rt );

	while(_rPos)
	{
		_pCtrl = m_ControlList.GetNext(_rPos);

		::MoveWindow( _pCtrl -> GetSafeHwnd(), rt.left+5, rt.top+25, rt.right-rt.left-14, rt.bottom-rt.top-34, TRUE );
...
void CSTabCtrl::CSTabPage::MoveWindow(int x, int y)
{
	POSITION _rPos = m_ControlList.GetHeadPosition();
	CWnd * _pCtrl = NULL;
...
CSTabCtrl::CPageToControlsMap::CPageToControlsMap(CSTabCtrl* pare)
{
	m_Parent = pare;
...
BOOL CSTabCtrl::CPageToControlsMap::AttachControl(INT _nTabNum,CWnd * _pControl)
{
	...
	{
		_pTabPage = new CSTabPage(m_Parent);
...
void CSTabCtrl::CPageToControlsMap::MoveWindows(int x, int y)
{
	...
	while(_rPos)
	{
		GetNextAssoc(_rPos,_rKey,_pTabPage);
		_pTabPage -> MoveWindow ( x, y );
	}
	return;
}
...
void CSTabCtrl::CPageToControlsMap::CloseAll()
{
	POSITION _rPos = GetStartPosition();
	CSTabPage * _pTabPage = NULL;
	INT _rKey;
...
CSTabCtrl::CSTabCtrl():m_TabPagesMap(this)
{
	m_nPrevSel = -1;
}
...
void CSTabCtrl::OnMove(int x, int y)
{
	m_TabPagesMap.MoveWindows(x, y);
}

void CSTabCtrl::Close(void)
{
	m_TabPagesMap.CloseAll();
}