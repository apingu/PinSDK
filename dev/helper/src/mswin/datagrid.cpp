...
#define dbgstrcpy  wcscpy
#define dbgstrlen  wcslen
#define dbgstrncpy wcsncpy


#else

#define dbgstrcpy strcpy
#define dbgstrlen strlen
#define dbgstrncpy strncpy

#endif
/* DataGrid definition */
/* DataGrid column definition structure */
typedef struct _tagDG_COL
{
...
void CDataGrid::AddNewCol( const TCHAR* text, int col, int width, int align )
{
	SetColumnInfo( col, (TCHAR*)text, width, align );
}

void CDataGrid::SetCellText( const TCHAR* text, int col, int row, int align, bool readOnly )
{
	SetItemInfo(row, col, (TCHAR*)text, align, readOnly);
}

void CDataGrid::AddRow( const TCHAR* text, int align )
{
	InsertItem( (TCHAR*)text, align );
}
...
BOOL CDataGrid::Create(RECT wndRect, HWND hParent, int numCols)
{
     ...
     wincl.lpszClassName = _T("DATAGRID");
     ...
     if ( ( reg ) || ( error == ERROR_CLASS_ALREADY_EXISTS ) )
     {
          ...
          m_hWnd = CreateWindowEx( WS_EX_CLIENTEDGE, _T("DATAGRID"), _T(""), WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_VSCROLL | WS_HSCROLL, 
          ...
          if ( dgList != NULL )
          {
               ...
               dgList->dg_Cols = new DG_COL[dgList->dg_ColumnNumber];
               for ( int i=0; i<dgList->dg_ColumnNumber; i++ )
               {
                   dgList->dg_Cols[i].columnWidth = 50;
                   dgList->dg_Cols[i].textAlign = DGTA_LEFT;
				   dbgstrcpy( dgList->dg_Cols[i].columnText, _T(" ") );
               }
               
               // Create EDIT control
               dgList->dg_hEditWnd = CreateWindow( _T("EDIT"), _T(""), WS_CHILD | WS_CLIPSIBLINGS | ES_AUTOHSCROLL, 0, 0, 100, 20, m_hWnd, NULL, GetModuleHandle(NULL), NULL );
...
void InitDGGlobals(HWND hParent, HWND hWnd)
{
     ...
     if ( dgList != NULL )
     {
         ...
         //dbgstrcpy( lf.lfFaceName, _T("Arial") );
		 dbgstrcpy( lf.lfFaceName, _T("微軟正黑體") );
		 ...
		 HDC hParentDC = GetDC(hParent);
		 HFONT font = (HFONT)SelectObject( hParentDC, GetStockObject(SYSTEM_FONT));
		 dgList->dg_hRowFont = font;
		 SelectObject( hParentDC, font);
		 ReleaseDC( hParent, hParentDC );
		 */
...
BOOL CDataGrid::SetColumnInfo(int columnIndex, TCHAR* columnText, int columnWidth, int textAlign)
{
     ...
     if ( dgList != NULL )
     {
         ...
         if ( columnIndex < dgList->dg_ColumnNumber )
         {
              ...
              dbgstrncpy( dgList->dg_Cols[columnIndex].columnText, columnText, 1024 );
              dgList->dg_Cols[columnIndex].columnWidth = columnWidth;
              dgList->dg_Cols[columnIndex].textAlign = textAlign;
              dgList->dg_Cols[columnIndex].pressed = false;
...
BOOL SetDGItemText(HWND hWnd, int rowIndex, int columnIndex, TCHAR* buffer)
{
     ...
     if ( dgList != NULL )
     {
         ...
         if ( ( columnIndex < dgList->dg_ColumnNumber ) && ( rowIndex < dgList->dg_RowNumber ) && ( buffer != NULL ) )
         {
              ...
              if ( dbgstrlen(dgList->dg_Rows[rowIndex].rowText[columnIndex]) < dbgstrlen(buffer) )
              {
                   delete dgList->dg_Rows[rowIndex].rowText[columnIndex];
                   dgList->dg_Rows[rowIndex].rowText[columnIndex] = new TCHAR[dbgstrlen(buffer)+1];
              }
              if ( dbgstrlen(buffer) == 0 )
                   dbgstrcpy( dgList->dg_Rows[rowIndex].rowText[columnIndex], _T(" ") );
              else
                   dbgstrcpy( dgList->dg_Rows[rowIndex].rowText[columnIndex], buffer );
...
BOOL GetDGItemText(HWND hWnd, int rowIndex, int columnIndex, TCHAR* buffer, int buffer_size)
{
     ...
     if ( ( columnIndex < dgList->dg_ColumnNumber ) && ( rowIndex < dgList->dg_RowNumber ) )
     {
          ...
          dbgstrncpy( buffer, dgList->dg_Rows[rowIndex].rowText[columnIndex], buffer_size );
     ...
     switch (message)
     {
        ...
        case WM_LBUTTONDOWN:
        {
             ...
             if ( dgList != NULL )
             {
                 ...
                 if ( dgList->dg_Click )
                      dgList->dg_Cols[dgList->dg_Column].pressed = true;
        ...
        case WM_LBUTTONUP:
        {
             ...
             {
                 for ( int i=0; i<dgList->dg_ColumnNumber; i++ )
                      dgList->dg_Cols[i].pressed = false;
                 ...
                 if ( wParam == MK_LBUTTON )
                 {
                      if ( ( dgList->dg_Resize ) && ( dgList->dg_EnableResize ) )
                      {
						   ...
						   if ( ( pt.x >= 0 ) && ( pt.x <= 2000 ) )
                           {
							   ...
							   dgList->dg_Cols[dgList->dg_Column].columnWidth = newSize;
							   RecalcWindow(hwnd);
...
void DrawCol(HWND hWnd)
{
     ...
     for ( int i=0; i<dgList->dg_ColumnNumber; i++ )
     {
         ...
         GetTextExtentPoint( hDC, dgList->dg_Cols[i].columnText, dbgstrlen(dgList->dg_Cols[i].columnText), &size );
         GetTextMetrics( hDC, &tm );
         
         // Set column rectangle
         rect.left = offsetX;
         rect.top = offsetY;
         rect.right = rect.left + dgList->dg_Cols[i].columnWidth;
         ...
         // Draw column text
         DrawTextEx( hDC, dgList->dg_Cols[i].columnText, dbgstrlen(dgList->dg_Cols[i].columnText), &rect, dgList->dg_Cols[i].textAlign | DT_VCENTER | DT_SINGLELINE | DT_WORD_ELLIPSIS, &dtp );
     }
...
void DrawRows(HWND hWnd)
{
     ...
     for ( int i=rowOffsetT; i<rowOffsetB; i++ )
     {
         for ( int j=0; j<dgList->dg_ColumnNumber; j++ )
         {
             ...
             GetTextExtentPoint( hDC, dgList->dg_Rows[i].rowText[j], dbgstrlen(dgList->dg_Rows[i].rowText[j]), &size );
             GetTextMetrics( hDC, &tm );
...
//                             hBgBrush = CreateSolidBrush(DGRONLY_COLOR);
                       ...
                       // Draw row text
                       DrawTextEx( hDC, dgList->dg_Rows[i].rowText[j], dbgstrlen(dgList->dg_Rows[i].rowText[j]), &rect, dgList->dg_Rows[i].textAlign[j] | DT_VCENTER | DT_SINGLELINE | DT_WORD_ELLIPSIS, &dtp );
                  }
                  else
                  {
                      // Draw row
...
void GetColumnRect(HWND hWnd, int col, RECT* colRect)
{
     ...
     for ( int i=0; i<dgList->dg_ColumnNumber; i++ )
     {
         ...
         if ( i == col )
         {
              ...
              colRect->top = offsetY;
              colRect->right = colRect->left + dgList->dg_Cols[i].columnWidth;
         ...
         offsetX += dgList->dg_Cols[i].columnWidth;
     }
     SelectObject( hDC, hOldFont );
...
void GetRowRect(HWND hWnd, int row, RECT* rowRect)
{
     ...
     {
           // Get row text dimensions
           GetTextExtentPoint( hDC, dgList->dg_Rows[i].rowText[0], dbgstrlen(dgList->dg_Rows[i].rowText[0]), &size );
           GetTextMetrics( hDC, &tm );
           ...
           if ( i == row )
           {
                ...
                rowRect->top = offsetY;
                rowRect->right = rowRect->left + dgList->dg_Cols[0].columnWidth;
...
BOOL CheckRows(HWND hWnd, int x, int y, int* row)
{
     ...
     *row = -1;
     ...
     for ( int j=0; j<dgList->dg_RowNumber; j++ )
     {
         ...
         {
             // Get row text dimensions
             GetTextExtentPoint( hDC, dgList->dg_Rows[j].rowText[i], dbgstrlen(dgList->dg_Rows[j].rowText[i]), &size );
             GetTextMetrics( hDC, &tm );
...
void RecalcWindow(HWND hWnd)
{
     ...
     if ( dgList != NULL )
     {
         ...
         for ( int i=0; i<dgList->dg_ColumnNumber; i++ )
         {
             ...
             sizeX += dgList->dg_Cols[i].columnWidth;
         }
...
BOOL CDataGrid::InsertItem(TCHAR* itemText, int textAlign)
{
     ...
     if ( dgList != NULL )
     {
         if ( dgList->dg_RowNumber < DG_MAXROW )
         {
              ...
              dgList->dg_Rows[dgList->dg_RowNumber-1].rowText = new TCHAR*[dgList->dg_ColumnNumber];
              ...
              for ( int j=0; j<dgList->dg_ColumnNumber; j++ )
              {
                  dgList->dg_Rows[dgList->dg_RowNumber-1].rowText[j] = new TCHAR[dbgstrlen(itemText)+1];
                  dbgstrcpy( dgList->dg_Rows[dgList->dg_RowNumber-1].rowText[j], _T(" ") );
                  dgList->dg_Rows[dgList->dg_RowNumber-1].textAlign[j] = textAlign;
                  dgList->dg_Rows[dgList->dg_RowNumber-1].readOnly[j] = false;
              }
              dbgstrcpy( dgList->dg_Rows[dgList->dg_RowNumber-1].rowText[0], itemText );
