...
#include "stdafx.h"
#include "mfc-sxbutton.h"    // Access non-local header (in a library)
...
BOOL CSXButton::SetMaskedBitmap(UINT nID,
                                 int nWidth,
                                 int nHeight,
                                 COLORREF crTransparentMask)
{
    return SetBitmapCommon(nID,
                            nWidth,
                            nHeight,
                            crTransparentMask,
                            TRUE /* mask */);
...
BOOL CSXButton::SetBitmapCommon(UINT nID,
                                 int nWidth,
                                 int nHeight,
                                 COLORREF crTransparentMask,
                                 BOOL bUseMask)
...
/*
                                                              HBITMAP bmTemp  = (HBITMAP)
                                                                                ::CreateMappedBitmap(AfxGetApp()->m_hInstance,
                                                                                                                  nID,
                                                                                                                  IMAGE_BITMAP,
                                                                                                                &mapColor,
                                                                                                                  bUseMask ?
    ...
    CBitmap bmTemp;
    bmTemp.LoadBitmap(nID);


    m_hBitmap = (HBITMAP) ::CopyImage(bmTemp,
                                         IMAGE_BITMAP,
                                         nWidth,
                                         nHeight,
                                         LR_COPYDELETEORG);
...
/*
                                                    HBITMAP bmTemp2   = (HBITMAP) ::CreateMappedBitmap(AfxGetApp()->m_hInstance,
                                                                                                nID,
                                                                                                IMAGE_BITMAP,
                                                                                                &mapColor,
                                                                                                bUseMask ? 1 : 0);*/
    ...
    CBitmap bmTemp2;
    //bmTemp2.LoadBitmap(nID);
    bmTemp2.LoadBitmap(nID);


    m_hBitmapDisabled = (HBITMAP) ::CopyImage(bmTemp2,
                                                 IMAGE_BITMAP,
                                                 nWidth,
                                                 nHeight,
                                                 LR_COPYDELETEORG);
...
void CSXButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if(!bDRAWFOCUSONLY)
    {
        ...
        //  Draw button frame
        //
        pDC->DrawFrameControl(&rectControl,
                               DFC_BUTTON,
                               DFCS_BUTTONPUSH | nFrameStyle);
...
#define COLORTABSIZE 8
            if(penTab.CreatePen(PS_SOLID, 1, m_crColorTab))
            {
                ...
                for(UINT nStep = 0;nStep < COLORTABSIZE;nStep++)
                {
                    ...
                    pDC->LineTo(nXOffset + (COLORTABSIZE - nStep) - 1,
                                 nYOffset + nStep);
                }
        ...
        if(HasImage())
        {
            ...
            if(m_hIcon)
                pDC->DrawState(pt,
                                CSize(m_nImageWidth, m_nImageHeight),
                                (HICON) m_hIcon,
                                DST_ICON | nStateFlag,
                                (CBrush *) NULL);
            else if(m_hBitmap)
