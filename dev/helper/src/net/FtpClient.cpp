...
const bool CFTPClient::DownloadWildcard(const std::string& strLocalDir, const std::string& strRemoteWildcard) const
{
   ...
   struct _stat64 info;
   if ( _stat64(data.strOutputPath.c_str(), &info) == 0 && (info.st_mode & S_IFDIR)) // S_ISDIR() doesn't exist on windows 
   {
                            ...
                            const bool& bCreateDir) const
{
   ...
   if ( _stat64(strLocalFile.c_str(), &file_info) == 0)
   {
