...
void ReleaseCsBrowser(HWND hwnd)
{
...
long UpdateHTMLStr(HWND hwnd, LPCTSTR string)
{    
...
long UpdateHTMLPage(HWND hwnd, LPTSTR webPageName)
{
    IWebBrowser2    *webBrowser2;
    VARIANT            myURL;
    IOleObject        *browserObject;
    ...
    if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
    {
        ...
        // TCHAR). The string is not nul-terminated. The OS function SysAllocString can allocate and
...
long CreateCsBrowser(HWND hwnd)
{
    IOleObject            *browserObject;
    IWebBrowser2        *webBrowser2;
    RECT                rect;
    ...
    if (!(ptr = (TCHAR *)GlobalAlloc(GMEM_FIXED, sizeof(IOleInPlaceFrameEx) + sizeof(_IOleClientSiteEx) + sizeof(IOleObject *))))
        return(-1);
