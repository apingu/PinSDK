...
FILE *curlx_win32_fopen( const char *filename, const char *mode )
{
    ...
    _tstring file_mode;
#ifdef _UNICODE
    file_name = pfstd::StringToUnicode( filename );
    file_mode = pfstd::StringToUnicode( mode );
#else
    file_name = filename;
    file_mode = mode;
#endif
    return (_tfopen)( file_name.c_str(), file_mode.c_str() );
}
...
#  pragma warning(disable:810) /* conversion may lose significant bits */
    ...
    assert( ulnum <= (unsigned long)CURL_MASK_UCHAR );
